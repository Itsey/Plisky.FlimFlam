...
namespace MexInternals {
    ...
    internal class DataStructureManager {
        ...
        } // End PlaceUnknownEventIntoDataStructure
        ...
        internal int PlaceNewEventIntoDataStructure(EventEntry newEvt, int thePid, string eventMachineName) {
            ...
            if ((newEvt.cmdType == TraceCommandTypes.InternalMsg) && (newEvt.SecondaryMessage.StartsWith(Constants.THREADINITIDENTIFIER))) {
                if (!MexCore.TheCore.DataManager.TracedApplications[physicalIndex].ThreadNames.ContainsKey(newEvt.CurrentThreadKey)) {
                    string tname = newEvt.SecondaryMessage.Substring(Constants.THREADINITIDENTIFIER.Length + 1);
        ...
        internal long GetNextGlobalIndex() {
            try {
                ...
                return ++m_currentGlobalIndex; // todo lock ?? shoudl be atomic
            } catch (OverflowException ex) {
               //Bilge.Log("Mex::DataManager - Global Index Overflow.  There were too many messages alocated and Mex cannot take any more.  Mex needs to be restarted");
               //Bilge.Log("Mex::DataManager - ERROR, Terminating : " + ex.Message);
                throw;
            }
        }
