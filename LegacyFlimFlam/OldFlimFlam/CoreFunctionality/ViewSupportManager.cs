...
using TimerInstanceDataStructure = System.Collections.Generic.Dictionary<string, System.Collections.Generic.List<MexInternals.TimeInstanceView>>;

using TimerPartialInstancedataStructure = System.Collections.Generic.Dictionary<string, System.Collections.Generic.List<MexInternals.TimeInstanceView>>;
...
namespace MexInternals {
    ...
    internal class ViewSupportManager {
        //public Bilge b;
        internal enum ExtendedDetailsMode { Exception, LogMessage, Error, AssertionFailure }

        private int m_selectedTracedApp = -1;
        ...
        internal int SelectedTracedAppIdx {
            get { return m_selectedTracedApp; }
            set { m_selectedTracedApp = value; }
        }

        internal TracedApplication SelectedTracedApp {
            get {
                if (m_selectedTracedApp > 0) {
                    return MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp);
                } else {
                    return null;
                }
            }
        ...
        internal string SelectedTracedAppName {
            get {
                // todo : cache for eprf
                return MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp).MachineName + "\\" +
                       MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp).ProcessName;
            }
        }
        ...
        internal string SelectedTracedAppComment {
            get {
                // tODO : perf
                return MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp).ProcessLabel;
            }
        }

        internal int SelectedTracedAppPid {
            // todo cache for perf?
            get { return MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp).ProcessIdNo; }
        }
        ...
        internal ProcessSummary SelectedTracedAppSummary {
            ...
            get {
                ...
                return new ProcessSummary(MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp));
            }
        }
        ...
        internal bool IsCurrentSelectedAppValid {
            get {
                if (m_selectedTracedApp == -1) { return false; } // Short circuit when its never been set up.
                return MexCore.TheCore.DataManager.IsValidTracedApplicationIndex(m_selectedTracedApp);  // Check to see its still valid
            }
        ...
        private List<string> m_userNotificationMessageLog = new List<string>();
        ...
        internal string LastUserNotificationMessage {
            get {
                if (m_userNotificationMessageLog.Count == 0) { return string.Empty; }
                return m_userNotificationMessageLog[m_userNotificationMessageLog.Count - 1];
            }
        }
        ...
        internal bool ApplyCurrentHighlightOptionsToEventEntry(EventEntry ee, int appIdx) {
            ...
            if (triggerRefresh && (appIdx == m_selectedTracedApp)) {
        ...
        internal List<string> GetAdditionalLocationClassesFromAdditionalLocations() {
            // TODO : implement this better (single loop) for performance if used frequently.
            List<string> results = GetAllAdditionalLocations();
        ...
        internal void CreateDuplicateOfSelectedApplication(string referenceName, long startIdx, long endIdx) {
            TracedApplication taDupe = MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp);
            TracedApplication taNew = new TracedApplication(taDupe, referenceName, startIdx, endIdx);
            MexCore.TheCore.DataManager.AddDuplicatedTracedApplication(taNew);
        }

        internal void CreateDuplicateOfSelectedApplication(string referenceName) {
            TracedApplication taDupe = MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp);
            TracedApplication taNew = new TracedApplication(taDupe, referenceName);
            MexCore.TheCore.DataManager.AddDuplicatedTracedApplication(taNew);
        }
        ...
        internal string GetMoreInfoForEventIndexInSelectedApp(long theIndex) {
           ...
           //Bilge.Log("MexViewer::ViewSupportManager::GetMoreInfoForEventIndex >> Looknig for further info");
            ...
            try {
                ...
                result += "Trace Entry Located in Module: " + ta.EventEntries[offsetOfIndex].Module + " @Line: " + ta.EventEntries[offsetOfIndex].LineNumber + " entered Mex at :" + ta.EventEntries[offsetOfIndex].m_timeMessageRecieved.ToString() + "\r\n";
                ...
                if ((ta.EventEntries[offsetOfIndex].SecondaryMessage != null) && (ta.EventEntries[offsetOfIndex].SecondaryMessage.Length > 0)) {
                    ...
                    if ((ta.EventEntries[offsetOfIndex].cmdType == TraceCommandTypes.TraceMessageIn) || (ta.EventEntries[offsetOfIndex].cmdType == TraceCommandTypes.TraceMessageOut)) {
                        DateTime dt = GetDateTimeFromTraceMessage(ta.EventEntries[offsetOfIndex].SecondaryMessage);
        ...
        internal string GetMoreInfoForEventIndexGlobally(long theIndex) {
           ...
           //Bilge.Log("MexViewer::ViewSupportManager::GetMoreInfoForEventIndex >> Looknig for further info");
            ...
            TracedApplication ta = MexCore.TheCore.DataManager.GetKnownApplication(tracedIdx);
            ta.EventEntries.EventEntriesRWL.AcquireReaderLock(Consts.MS_TIMEOUTFORLOCKS);
            try {
                int tempOffset = -1;  // Used to move around the event entries
        ...
        public int ImageIndexFromEventType(TraceCommandTypes tct) {
            switch (tct) {
                case TraceCommandTypes.LogMessage: return 32;
                case TraceCommandTypes.LogMessageVerb: return 34;
                case TraceCommandTypes.InternalMsg: return 2;
                case TraceCommandTypes.TraceMessageIn: return 15;
                case TraceCommandTypes.TraceMessageOut: return 17;
                //case TraceCommandTypes.TraceMessage: return 19;
                case TraceCommandTypes.AssertionFailed: return 13;
                case TraceCommandTypes.MoreInfo: return 7;
                case TraceCommandTypes.CommandOnly: return 0;
                case TraceCommandTypes.ErrorMsg: return 21;
                case TraceCommandTypes.WarningMsg: return 37;
                case TraceCommandTypes.ExceptionBlock: return 22;
                case TraceCommandTypes.ExceptionData: return 23;
                case TraceCommandTypes.ExcStart: return 23;
                case TraceCommandTypes.ExcEnd: return 23;
                case TraceCommandTypes.SectionStart: return 6;
                case TraceCommandTypes.SectionEnd: return 6;
                case TraceCommandTypes.ResourceEat: return 4;
                case TraceCommandTypes.ResourcePuke: return 3;
                case TraceCommandTypes.ResourceCount: return 36;
                default:
                    throw new InvalidOperationException("There is an unknown entry that cant be resolved to an image");
            }
        ...
        internal DateTime GetDateTimeFromTraceMessage(string traceMessage) {
            ...
            string[] splitone = traceMessage.Substring((TMRIDENT.Length + 4)).Split('|');
            try {
                ...
                DateTime enteredTime = new DateTime(yy, mm, dd, hh, mi, ss, ms);
                return enteredTime;
            } catch (FormatException fex) {
               //Bilge.Dump(fex, "Excepton occured when trying to parse the date time on a timer in, ignoring.");
            }
            return DateTime.MinValue;
        }
        ...
        internal void RefreshView_ProcessTree(TreeView tvwTheView, KeyDisplayRepresentation threadIdentityToWorkOn) {
            ...
            try {
               ...
               //Bilge.Log(MexCore.TheCore.Options.RespectFilter ? "Filter is being respected." : "Filter is not being respected.");
                ...
                currentParent.ImageIndex = 0;
                tvwTheView.Nodes.Add(currentParent);

                foreach (EventEntry ee in ta.EventEntries) {
                    parsed++;

                    if (ee.CurrentThreadKey != threadIdentityToWorkOn.KeyIdentity) continue;  // Tree view only works on one thread at a time
                    ...
                    if (ee.cmdType == TraceCommandTypes.TraceMessageIn) {
                        if (ee.SecondaryMessage != null) {
                            DateTime dt = GetDateTimeFromTraceMessage(ee.SecondaryMessage);
                        ...
                        currentParent.Nodes.Add(nextNode);
                    ...
                    if (ee.cmdType == TraceCommandTypes.TraceMessageOut) {
                        if (ee.SecondaryMessage != null) {
                            ...
                            if (dt != DateTime.MinValue) {
                                nextNode.Tag = dt;
                                TimeSpan elapsed = dt - ((DateTime)currentParent.Tag);
        ...
        internal void MaskEventEntriesUpTillGlobalIndex(long idx) {
            ...
            for (int i = 0; i < ta.EventEntries.Count; i++) {
                if (ta.EventEntries[i].GlobalIndex == idx) break;
                ta.EventEntries[i].LastVisitedFilterResult = false;
            }
        }
                ...
                try {
                   ...
                   //Bilge.Log("Process View  refresh called for " + (ta.EventEntries.Count - loopStartOffset).ToString() + " elements.  If this is ever zero its a wasted refresh.");
                    ...
                    for (int loopCnt = loopStartOffset; loopCnt < ta.EventEntries.Count; loopCnt++) {
                        ...
                        if (colsShowGlobalIndex) {
                            subStringElements[ssidx++] = ee.GlobalIndex.ToString();
                        } else {
                            subStringElements[ssidx++] = string.Empty;
                        }
        ...
        internal void RefreshView_CrossProcess(ListView lvwDisplay, int[] virtualIndexedPids, bool incremental, long forceVisibleIndex) {
            ...
            try {
                ...
                for (int appCount = 0, nextApp = virtualIndexedPids[0]; appCount < virtualIndexedPids.Length; appCount++) {
                    ...
                    TracedApplication ta = MexCore.TheCore.DataManager.GetKnownApplication(nextApp);
                    ta.EventEntries.EventEntriesRWL.AcquireReaderLock(Consts.MS_TIMEOUTFORLOCKS);
        ...
        private Color GetColorByIndex(int p) {
            switch (p) {
                case 0: return Color.PaleGoldenrod;
                case 1: return Color.PaleGreen;
                case 2: return Color.PeachPuff;
                case 3: return Color.BlanchedAlmond;
                case 4: return Color.LightCoral;
                case 5: return Color.LightSteelBlue;
                case 6: return Color.LightSalmon;
                case 7: return Color.LightGray;
                case 8: return Color.IndianRed;
                case 9: return Color.Honeydew;
        ...
        internal List<KeyDisplayRepresentation> GetThreadNameListForSelectedApp() {
            List<KeyDisplayRepresentation> kdrs = new List<KeyDisplayRepresentation>();
            ...
            if (ta != null) {
               ...
               //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
                try {
                    foreach (EventEntry ee in ta.EventEntries) {
                        KeyDisplayRepresentation next = ta.GetThreadDisplayValue(ee);
        ...
        internal void RefreshView_SourceSynch(ListView lvwSourceView, string threadFilter) {
            ...
            try {
                ...
                lvwSourceView.Columns.Add("Index");
               ...
               //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
                try {
                    foreach (EventEntry ee in ta.EventEntries) {
                        ...
                        ListViewItem lvi = new ListViewItem(ee.GlobalIndex.ToString());
                        lvi.Tag = ee.GlobalIndex;
                        lvi.ImageIndex = ImageIndexFromEventType(ee.cmdType);
        ...
        internal void RefreshView_ProcessThread(ListView lvwDrawThreads, List<KeyDisplayRepresentation> includeTheseThreads, bool synch) {
            ...
            try {
               ...
               //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
                try {
                    ThreadColumnWidth = AvailableWidth / includeTheseThreads.Count;
                    ...
                    for (int i = 0; i < includeTheseThreads.Count; i++) {
                        ...
                        string name;
                        if (ta.ThreadNames.ContainsKey(includeTheseThreads[i].KeyIdentity)) {
                            name = ta.ThreadNames[includeTheseThreads[i].KeyIdentity];
                        } else {
                            name = includeTheseThreads[i].DisplayIdentity;
                        }
                        lvwDrawThreads.Columns.Add(name, name);
                    ...
                    foreach (EventEntry ee in ta.EventEntries) {
                        ...
                        for (int idx = 0; idx < includeTheseThreads.Count; idx++) {
                            if (idx == columnMatchIndex) {
                                lvi.SubItems.Add(BeautifyDebugMessage(ee.DebugMessage));
                            } else {
                                lvi.SubItems.Add("");
                            }
                        }

                        lvwDrawThreads.Items.Add(lvi);
                    }
                } finally {
                   //Bilge.ResourceFree(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
                    ta.EventEntries.EventEntriesRWL.ReleaseReaderLock();
           ...
           //Bilge.ResourceGrab(MexCore.TheCore.DataManager.NonTracedApplicationEntries.NonTracedApplicationsDataRWL, "NonTracedApplicationsDataRWL");
            try {
                ...
                for (long loop = startingIdx; loop < MexCore.TheCore.DataManager.NonTracedApplicationEntries.Count; loop++) {
                    NonTracedApplicationEntry nta = MexCore.TheCore.DataManager.NonTracedApplicationEntries[(int)loop];
        ...
        internal ResourceProfile[] GetAppResourceProfileToIndex(long idx) {
            Hashtable ht = new Hashtable();  // use this to build up the results
           ...
           //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
            try {
                foreach (EventEntry ee in ta.EventEntries) {
                    ...
                    NameValuePair[] resByIdx = GetResourceCounts(ee);
                    foreach (NameValuePair nvp in resByIdx) {
                        if (!ht.Contains(nvp.Name)) {
                            ht.Add(nvp.Name, new ArrayList());
                        }

                        (ht[nvp.Name] as ArrayList).Add(new ValueIdxPair(ee.GlobalIndex, nvp.Value));
                    } // end foreach nvp
            ...
            ResourceProfile[] result = new ResourceProfile[ht.Count];
            int resProfileLoopCount = 0;

            foreach (string s in ht.Keys) {
                result[resProfileLoopCount] = new ResourceProfile();
                result[resProfileLoopCount].Name = s;
                result[resProfileLoopCount].Consumption = (ValueIdxPair[])((ArrayList)ht[s]).ToArray(typeof(ValueIdxPair));
                // now run through the array finding lowest and highest

                result[resProfileLoopCount].HighestValue = long.MaxValue;
                result[resProfileLoopCount].LowestValue = long.MinValue;
        ...
        private NameValuePair[] GetResourceCounts(EventEntry ee) {
            ...
            if (splitter.EndsWith(";")) { splitter = splitter.Substring(0, splitter.Length - 1); }

            string[] working = splitter.Split(';');

            NameValuePair[] result = new NameValuePair[working.Length];
        ...
        internal NameValuePair[] GetResourceCountsByIndex(long globalIndex) {
            return GetResourceCounts(MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp).FindEventByGlobalIndex(globalIndex));
        }
        ...
        private static bool ParseEntryForTimingData(Dictionary<string, List<TimeInstanceView>> monitoredTimerInstances, List<TimeInstanceView> existingPartials, EventEntry ee) {
            ...
            TimerInstanceData tid = (ee.SupportingData as TimerInstanceData);
            if (tid == null) { return true; }  // This section did not support timing data therefore theres nothing we can do.

            TimeInstanceView tiv;

            if (ee.cmdType == TraceCommandTypes.SectionStart) {
                tiv = new TimeInstanceView();
                tiv.SinkName = tid.TimeSinkName;
                tiv.InstanceName = tid.TimeSinkInstanceDescription;
                tiv.EnterTime = tid.TimeInstance;
        ...
        private static void RenderTimings(TreeView tvwTimingsReport, Dictionary<string, List<TimeInstanceView>> monitoredTimerInstances, Dictionary<string, List<TimeInstanceView>> partialMatchedEntries, AdditionalData additional) {
            TreeNode baseNode = new TreeNode("Timings");
            baseNode.Name = "bnMatched";
            ...
            try {
                ...
                tvwTimingsReport.Nodes.Add(baseNode);

                foreach (string s in monitoredTimerInstances.Keys) {
                    List<TimeInstanceView> ltiv = monitoredTimerInstances[s];
                    categoryNode = new TreeNode();  // Timings  /  Category  << this is the category being created.
                    TimeSpan totalCategoryElapsed = new TimeSpan();
                    List<TreeNode> nodesStashed = new List<TreeNode>();
                    int filteredNodeCount = 0;

                    // Now do the instances.
                    foreach (TimeInstanceView tiv in ltiv) {
                        bool autoInstance = false;
                        string thisInstanceName = tiv.InstanceName;

                        if (thisInstanceName.StartsWith(Constants.AUTOTIMER_PREFIX)) {
                            thisInstanceName = thisInstanceName.Substring(Constants.AUTOTIMER_PREFIX.Length) + " (Auto)";
                            autoInstance = true;
                        }

                        instanceNode = new TreeNode();

                        TimeSpan elapsedForInstance = TimeSpan.MinValue;
                        ...
                        try {
                            ...
                            instanceNode.Tag = (double)elapsedForInstance.TotalMilliseconds;
                        ...
                        instanceNode.Nodes.Add(detailNodes);
                    ...
                    if (nodesStashed.Count > 2) {
                        ...
                        double running = 0;
                        double maxMili = (double)nodesStashed[nodesStashed.Count - 1].Tag;
                ...
                baseNode.Name = "bnUnmatched";
                tvwTimingsReport.Nodes.Add(baseNode);

                foreach (List<TimeInstanceView> ltiv in partialMatchedEntries.Values) {
                    foreach (TimeInstanceView unmatched in ltiv) {
                        instanceNode = new TreeNode(unmatched.InstanceName + " @ " + unmatched.SinkName);
                        instanceNode.Nodes.Add(unmatched.EnterTime.ToShortTimeString());
                        instanceNode.Nodes.Add(unmatched.ExitTime.ToShortTimeString());
        ...
        internal void RefreshView_Timings(TreeView tvwTimingsReport, AdditionalData additional) {
            ...
            TimerInstanceDataStructure monitoredTimerInstances = new Dictionary<string, List<TimeInstanceView>>();
           ...
           //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
            try {
                ...
                foreach (EventEntry ee in ta.EventEntries) {
                    ...
                    if (TraceMessageFormat.IsSectionCommand(ee.cmdType)) {
                        ...
                        ParseEntryForTimingData(monitoredTimerInstances, partialEntriesDictionary[ee.CurrentThreadKey], ee);
                    }
                } // End foreach
            ...
            TreeNode tn = tvwTimingsReport.Nodes["bnMatched"];
        ...
        internal void RefreshView_Resources(TreeView tvwTrackedResource, bool incremental) {
            ...
            Dictionary<string, List<ResourceTrackingInstance>> tracedResourceCountInstances = new Dictionary<string, List<ResourceTrackingInstance>>();
            Dictionary<string, List<ResourceTrackingInstance>> countedResourceCountInstances = new Dictionary<string, List<ResourceTrackingInstance>>();
        ...
        private static void ParseEntryForResourceInformation(Dictionary<string, List<ResourceTrackingInstance>> tracedResourceCountInstances, Dictionary<string, List<ResourceTrackingInstance>> countedResourceCountInstances, EventEntry ee) {
            ...
            if ((ee.cmdType == TraceCommandTypes.ResourcePuke) || (ee.cmdType == TraceCommandTypes.ResourceEat)) {
                ...
                if ((ee.SupportingData == null) || (!(ee.SupportingData is ResourceInstanceData))) {
            ...
            if (ee.cmdType == TraceCommandTypes.ResourceCount) {
                ...
                ResourceTrackingInstance rti = new ResourceTrackingInstance();
                rti.Name = ((ResourceInstanceData)ee.SupportingData).ResourceName;
                rti.Value = ((ResourceInstanceData)ee.SupportingData).ResourceValue;
                rti.GlobalIndex = ee.GlobalIndex;
                rti.Line = ee.LineNumber;
                rti.Module = ee.Module;
                rti.MoreInfoData = ee.MoreLocationData;
        ...
        private static void RenderResources(TreeView tvwNumericResources, Dictionary<string, List<ResourceTrackingInstance>> tracedResourceCountInstances, Dictionary<string, List<ResourceTrackingInstance>> numericCountInstances, bool colorNodes) {
            TreeNode tracked = new TreeNode("Tracked Resources");
            TreeNode counted = new TreeNode("Numeric Resources");
            ...
            try {
                ...
                foreach (string s in tracedResourceCountInstances.Keys) {
                    TreeNode resNode = new TreeNode(s);
                    tracked.Nodes.Add(resNode);
                    long runningTotalForThisResource = 0;
                    Dictionary<string, TreeNode> treeNodeByContextStore = new Dictionary<string, TreeNode>();

                    foreach (ResourceTrackingInstance rti in tracedResourceCountInstances[s]) {
                        TreeNode t;
                        if (!treeNodeByContextStore.ContainsKey(rti.Context)) {
                            t = new TreeNode();
                            t.Text = "Changes for context: " + rti.Context;
                        ...
                        if (!treeNodeByContextStore.ContainsKey("All Contexts")) {
                            t = new TreeNode();
                            t.Text = "Changes for resource across all contexts";
                        ...
                        TreeNode contextInstanceCount = new TreeNode("Value Changed By (" + rti.Value.ToString() + ") [" + rti.SupportingData + "] - Total >" + runningTotalForThisResource.ToString());
                        treeNodeByContextStore[rti.Context].Nodes.Add(contextInstanceCount);

                        TreeNode contextInstance = new TreeNode("Value Changed By (" + rti.Value.ToString() + ")   >  " + rti.Context + "[" + rti.SupportingData + "] - Total >" + runningTotalForThisResource.ToString());
                        contextInstance.Tag = rti;
                        ...
                        treeNodeByContextStore[rti.Context].Nodes.Add(contextInstanceCount);
                        treeNodeByContextStore["All Contexts"].Nodes.Add(contextInstance);
                    }
                ...
                foreach (string s in numericCountInstances.Keys) {
                    TreeNode nextResource = new TreeNode(s);
                    counted.Nodes.Add(nextResource);
                    long lastValueToDeltaWith = 0;
                    long deltaTotalForThisResource = 0;

                    foreach (ResourceTrackingInstance rti in numericCountInstances[s]) {
                        TreeNode contextInstance = new TreeNode("Value = " + rti.Value + "   >  " + rti.SupportingData);
                        contextInstance.Tag = rti;
                        ...
                        nextResource.Nodes.Add(contextInstance);
                    }
                }

                tvwNumericResources.Nodes.Add(tracked);
                tvwNumericResources.Nodes.Add(counted);
        ...
        #endregion  //End resource view region
        ...
        private int GetPreferredDetailsIndex(TracedApplication ta, int eeidx) {
            ...
            EventEntry ae = ta.EventEntries[eeidx];
        ...
        private int GetNextExceptionIndex(int eeidx, TracedApplication ta) {
            EventEntry ee = ta.EventEntries[eeidx];
        ...
        internal void PopulateLogMessageDetailsScreen(long idx, TextBox txtDebugEntryMain, TextBox txtDebugEntrySecondary, TextBox txtDebugEntryMore) {
           ...
           //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
            try {
                ...
                txtDebugEntrySecondary.Text = ee.SecondaryMessage;
        ...
        internal void PopulateExceptionDetailsScreen(ListBox lbxExceptionStack, TextBox txtPrimaryMessage, TextBox txtLocationFilename, TextBox txtLineNo, long idx) {
           ...
           //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
            try {
                ...
                while (nidx > 0) {// Cant be zero as it must have started somewhere
                    ...
                    if (ee.cmdType == TraceCommandTypes.ExceptionData) {
                        ...
                        if (ee.DebugMessage.StartsWith("H:")) {
                            llae.HelpURL = ee.DebugMessage.Substring(2);
                        } else if (ee.DebugMessage.StartsWith("R:")) {
                            llae.Source = ee.DebugMessage.Substring(2);
                        } else {
                            llae.MoreStuffAboutIt += ee.DebugMessage + Environment.NewLine;
                        }

                        if (ee.SecondaryMessage.StartsWith("T:")) {
                            llae.TargetSite = ee.SecondaryMessage.Substring(2);
                        } else if (ee.SecondaryMessage.StartsWith("S:")) {
                            llae.StackTrace = ee.SecondaryMessage.Substring(2);
                        } else {
                ...
                llae = llaeTop;
                while (llae != null) {
                    lbxExceptionStack.Items.Add(llae);
                    llae = llae.InnerException;
                }
            } finally {
               //Bilge.ResourceFree(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
           ...
           //Bilge.ResourceGrab(ta.EventEntries.EventEntriesRWL, "EventEntriesRWL");
            try {
                ...
                while (backTrackCount < BACKTRACKCOUNTLIMIT) {
                    lbxBackTraceDetails.Items.Add(string.Format("Exception-{0,-2}: {1,-15} on thread ({2,-4}): {3}", backTrackCount, ta.EventEntries[eeidx].cmdType.ToString(), ta.EventEntries[eeidx].ThreadID, BeautifyDebugMessage(ta.EventEntries[eeidx].DebugMessage)));
        ...
        private bool EveryEntryInOrderCallbackForTimedView(EventEntry ee, NonTracedApplicationEntry nta, int indexOfApp) {
            ...
            if (ee != null) {
                ...
                if ((!MexCore.TheCore.Options.RespectFilter) || (CurrentFilter.IncludeThisEventEntry(ee))) {
                    ...
                    lvi.SubItems.Add(MexCore.TheCore.DataManager.GetKnownApplication(indexOfApp).ProcessIdAsString);
                    lvi.SubItems.Add(ee.DebugMessage);
                    CurrentHighlightOptions.ModifyEventEntryForHighlight(ee);
                    ...
                    m_listViewToFillForTimedView.Items.Add(lvi);
                    return false;
                }
            }
            if (nta != null) {
                ...
                if ((!MexCore.TheCore.Options.RespectFilter) || (CurrentFilter.IncludeThisNonEventEntry(nta))) {
                    if (CurrentHighlightOptions.ModifyNonTracedEventEntryForHighlight(nta)) {
                        ...
                        lvi.SubItems.Add(nta.Pid.ToString());
                        lvi.SubItems.Add(nta.DebugEntry);
        ...
        internal void RefreshView_Timed(ListView lvwView, bool incremental) {
            ...
            try {
                ...
                if (!MexCore.TheCore.DataManager.VisitEveryEventEntryInOrder(new DataStructureManager.VisitEachEntryCallback(EveryEntryInOrderCallbackForTimedView), null)) {
                    LastViewSummary = "Refresh Cancelled";
                } else {
                    LastViewSummary = "Refresh Done.";
                }
        ...
        internal void RefreshView_Diagnostics(Label lbl, TextBox output, long deepInspectIndex) {
           ...
           //Bilge.Log("Going to get dianostics text from the datamanager");
            ...
            EventEntry ee = MexCore.TheCore.DataManager.FindEventEntryInKnownAppsByIndex(deepInspectIndex);

            if (ee == null) {
                NonTracedApplicationEntry nta = MexCore.TheCore.DataManager.FindNTAEntryByIndex(deepInspectIndex);

                if (nta == null) {
                    deepInspectString = "No match found for index.";
                } else {
                    deepInspectString = nta.GetDiagnosticStringData();
                }
            } else {
        ...
        internal void RequestSelectedAppPurge() {
           //Bilge.Log("View Requesting Operation  >> Purge Selected Requested");
            ...
            MexCore.TheCore.WorkManager.AddJob(new Job_PurgeTracedAppByIndex(this.m_selectedTracedApp));
            this.SelectedTracedAppIdx = -1;
        }
        ...
        internal void RequestPurgeAllExceptCurrentlySelected() {
           //Bilge.Log("View Requesting Operation >> Purge AllExceptCurrentlySelected requested");
            AddUserNotificationMessageByIndex(UserMessages.PurgeJobStarts, UserMessageType.InformationMessage, null);
            MexCore.TheCore.WorkManager.ProcessJob(new Job_PurgeAllData(m_selectedTracedApp));
        }
        ...
        internal void RequestSelectedAppClear() {
           //Bilge.Log("View Requesting Operation  >> SelectedAppPartialPurgeRequested");
            MexCore.TheCore.WorkManager.ProcessJob(new Job_PartialPurgeApp(m_selectedTracedApp));
        }
        ...
        internal void ProcessNewEventNotification(object affectedIndex) {
            ...
            if ((idx == this.SelectedTracedAppIdx) || (idx == -1)) {
               ...
               //Bilge.LogIf(RegisterForSelectedProcessRefreshRequired == null, "Mex::ViewSupportManager::ProcessNewEventNotification - No one registered for selected app changes, skipping notification");
                if (RegisterForSelectedProcessRefreshRequired != null) {
                    RegisterForSelectedProcessRefreshRequired(true);
                }
            }
        }
        ...
        internal void ProcessChangeNotification(object selectedProcess) {
           ...
           //Bilge.LogIf(RegisterForProcessListChanges == null, "Mex::ViewSupportManager::ProcessChangeNotification -No one registerd for any process list changes, skipping all process changed notification");
            ...
            if (RegisterForProcessListChanges != null) {
                ...
                // process summaries.
                ProcessSummary[] pss = MexCore.TheCore.DataManager.GetAllProcessSummaries();
           ...
           //Bilge.LogIf(selectedProcess == null, "Mex::ViewSupportManager::ProcessChangeNotification -SelectedProcess param is null, skipping selectedPRocessChanged notification");
            ...
            if ((selectedProcess != null) && (RegisterForSelectedProcessChanges != null)) {
                try {
                    ...
                    if (theIdx == this.m_selectedTracedApp) {
     ...
     //Bilge.Log("Unhandled exception occured in MEX " + e.Exception.Message);
      ...
      Application.ThreadException += new System.Threading.ThreadExceptionEventHandler(Application_ThreadException);
        ...
        internal bool ApplyCurrentHighlightOptionsToView() {
            if (m_selectedTracedApp < 0) { return false; }
        ...
        internal AssertionPopulationData GetAssertionFurtherInformation(long idx) {
            AssertionPopulationData result = new AssertionPopulationData();

            TracedApplication ta = MexCore.TheCore.DataManager.GetKnownApplication(m_selectedTracedApp);
            int assertionOffset = ta.EventEntries.ContainsThisGlobalIdx(idx);
            EventEntry ee = ta.EventEntries[assertionOffset];
            ...
            for (int i = assertionOffset + 1; i < ta.EventEntries.Count; i++) {
                EventEntry further = ta.EventEntries[i];
        ...
        private void ExtractFurtherAssertionInfoFromMoreInfo(AssertionPopulationData result, EventEntry further) {
            ...
            string intialMarker = Constants.ASSERTIONRECREATESTRING + "STK:";
            if (stackData.StartsWith(intialMarker)) {
                result.Stacktrace = stackData.Substring(intialMarker.Length);
            } else {
                result.Stacktrace = "Unknown.";
            }
        }
        ...
        internal ErrorDialogData GetErrorFurtherInformation(long idx) {
            ErrorDialogData result = new ErrorDialogData();
