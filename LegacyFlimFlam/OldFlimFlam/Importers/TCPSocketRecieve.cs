           ...
           //Bilge.E("TCPReciever Thread / thread Loop");
            try {
               //Bilge.Log("Brinigng TCP Listener Online");
                ...
                if (MexCore.TheCore.Options.IPAddressToBind == "127.0.0.1") {
                   //Bilge.Warning("Listening on local port only");
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPStatusMessage, ViewSupportManager.UserMessageType.WarningMessage, "The TCP listener is bound to the local machine only.");
                }
                ...
                if (MexCore.TheCore.Options.IPAddressToBind == "*") {
                    ...
                    try {
                       ...
                       //Bilge.Dump(sox, "Socket Exception resolving the hostname entered, not starting TCP listener");
                        MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPListenerInvalidHostError, ViewSupportManager.UserMessageType.ErrorMessage, "IP:" + MexCore.TheCore.Options.IPAddressToBind);
               ...
               //Bilge.FurtherInfo("Binding to " + MexCore.TheCore.Options.IPAddressToBind + " on port " + port.ToString());
                ...
                try {
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPStatusMessage, ViewSupportManager.UserMessageType.InformationMessage, "The TCP listener is starting.");
                    server.Start();
                   ...
                   //Bilge.Warning("Socket exception, this instance of Mex is not listening for incomming connections");
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPStatusMessage, ViewSupportManager.UserMessageType.WarningMessage, "The TCP listener was terminated due to an error." + sex.Message);
                }
                ...
                try {
                    ...
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPStatusMessage, ViewSupportManager.UserMessageType.ErrorMessage, "The TCP listener has errored and is closing down." + ex.Message);
                } finally {
                    server.Stop();
                }
            } finally {
            ...
            do {
                try {
                   ...
                   //Bilge.Dump(iex, "Network connection closed, unable to retrieve more data");
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.TCPStatusMessage, ViewSupportManager.UserMessageType.InformationMessage, "The connection was forcibly closed by the other end of the TCP Stream. Connection Aborted");
                    return;
                ...
                int liMarker = temp.LastIndexOf(Constants.TCPEND_MARKERTAG);

                if (liMarker > -1) {
                    incomingMessage.Length = 0;
                    incomingMessage.Append(temp.Substring(liMarker + Constants.TCPEND_MARKERTAGLEN));

                    string[] msgs = temp.Substring(0, liMarker).Split(new string[1] { Constants.TCPEND_MARKERTAG }, StringSplitOptions.RemoveEmptyEntries);

                    foreach (string s in msgs) {
                        IncomingMessageManager.Current.AddIncomingMessage(s, -1);
                    }
                }
            } while (bytesRead != 0);
