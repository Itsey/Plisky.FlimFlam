        ...
        internal static void InterceptODS() {
           //Bilge.Log("InterceptODS Thread started");
            ...
            { // Reads ODS Data from Swapfile
                ...
                hBufferReadyEvent = (IntPtr)0;
                hDataReadyEvent = (IntPtr)0;
                hSwapFileBuffer = (IntPtr)0;
                ...
                fixed (byte* securityDescr = tmem) {
                    try {
                        ...
                        ODSWin32Interface.SECURITY_ATTRIBUTES* psecurityAttr = &securityAttr;
                        ...
                        while (continueRunning) {
                            try {
                               ...
                               //Bilge.TimeStart("ODSCapture", "ODS");
                                ...
                                byte* findLen = (byte*)pcharToDebugData;
                                int strlenctr = 0;
                                ...
                                string s = new string((sbyte*)pcharToDebugData, 0, strlenctr, System.Text.Encoding.Default);
                               //Bilge.TimeStop("ODSCapture", "ODS");

                                if (s.Length > 0) {
                                   //Bilge.TimeStart("AddIncomming", "ODS");
                                    MexCore.TheCore.MessageManager.AddIncomingMessage(s, pid);
                               ...
                               //Bilge.Dump(ex, "MEX::ODSGatherer::InterceptODS Error, ignoring error");
                                MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.ODSStatusMessage, ViewSupportManager.UserMessageType.WarningMessage, "The ODS Listener experienced an error and has lost a message.");
                            }
                       ...
                       //Bilge.Log("Disabling ODS listener, no ODS messages will be captured");
                        MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.ODSStatusMessage, ViewSupportManager.UserMessageType.ErrorMessage, "The ODS listener was unable to initialise the access to the swap file.  Possibly 64Bit machine?");
                    } finally {
                        ...
                        if (hDataReadyEvent != (IntPtr)0) { ODSWin32Interface.CloseHandle(hDataReadyEvent); }
                        if (hSwapFileBuffer != (IntPtr)0) { ODSWin32Interface.CloseHandle(hSwapFileBuffer); }
