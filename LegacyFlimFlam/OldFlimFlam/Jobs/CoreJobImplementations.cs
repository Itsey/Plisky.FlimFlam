...
namespace MexInternals {
        ...
        internal override void ExecuteJob() {// TODO : look for any and all of the same jobs on the queue
           ...
           //Bilge.Warning("Mex::WorkManager >> WARNING >> Should check for duplicate jobs here.  TODO");
            ...
            if (MexCore.TheCore.MessageManager.MessagesPendingImport > MexCore.TheCore.Options.MessagesToSpawnThreadsFor) {
                ThreadPool.QueueUserWorkItem(new WaitCallback(MexCore.TheCore.MessageManager.BackgroundProcessAllMessages), null);
    ...
    internal class Job_ApplyHighlightToView : BaseJob {
        ...
        internal override void ExecuteJob() {
           //Bilge.Log("Mex::WorkManager >> Found apply highlight to structures request, applying highlight now");
            ...
            MexCore.TheCore.ViewManager.ApplyCurrentHighlightOptionsToView();
            MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.HighlightOperationRequestsViewRefresh, ViewSupportManager.UserMessageType.InformationMessage, null);
    ...
    internal class Job_ApplyHighlightToStructures : BaseJob {
        ...
        internal override bool InitialiseJob(out bool requiresNotificationSuspense) {
            requiresNotificationSuspense = true;
            MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.BackgroundHighlightOperationBegins, ViewSupportManager.UserMessageType.BackgroundProcessingMessage, null);
        ...
        internal override void ExecuteJob() {
           //Bilge.Log("Mex::WorkManager >> Found apply highlight to structures request, applying highlight now");
            MexCore.TheCore.DataManager.VisitEachKnownApplicationEventEntry(new DataStructureManager.VisitEachEventEntryCallback(MexCore.TheCore.ViewManager.ApplyCurrentHighlightOptionsToEventEntry), new DataStructureManager.VisitEachEventryCompletedCallback(MexCore.TheCore.ViewManager.HighlightUpdateComplete));
        }

        internal override void PerformPostJob() {
            MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.BackgroundHighlightOperationEnds, ViewSupportManager.UserMessageType.BackgroundProcessingMessage, null);
        }
