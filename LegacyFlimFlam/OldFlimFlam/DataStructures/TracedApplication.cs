...
namespace MexInternals {
    ...
    internal class TracedApplication {
        ...
        internal EventEntryArrayList EventEntries;  // TODO : Replace this with something less boxy - it must be perf hit
        ...
        internal bool ContainsTraceData;                // True if any entry contains trace data.
        internal bool ContainsResourceData;             // True if any event entry contains resource data
        internal bool ContainsTimingData;               // True if any event entry contains timing data
        internal bool ContainsMoreThanOneThread;        // True if more than one thread is in the event entries
        internal string CurrentlyUsedThreadIdx = string.Empty;  // Holds the first thread idx to be used, if it doesent match on event add then the above is true
