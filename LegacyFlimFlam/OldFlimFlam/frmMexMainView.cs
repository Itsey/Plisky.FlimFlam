...
namespace MexInternals {
    ...
    public partial class frmMexMainView : Form {
        ...
        private List<LinkLabel> processLinkLabelCache = new List<LinkLabel>();
        ...
        public frmMexMainView() {
            InitializeComponent();
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select All", null, tsHandler_Click));
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select None", null, tsHandler_Click));
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select All Starting With", null, tsHandler_Click));
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Clear then select all starting with:", null, tsHandler_Click));
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select All Not Starting With", null, tsHandler_Click));
            sptSelector.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Experimental Purge", null, tsHandler_Click));

            spbtSelectThreads.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select All", null, tsThreadViewHandler_Click));
            spbtSelectThreads.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Select None", null, tsThreadViewHandler_Click));
        ...
        private void sptSelector_Click(object sender, EventArgs e) {
            ToolStripMenuItem tsmi = new ToolStripMenuItem("Select All", null, tsHandler_Click);
        ...
        private void tsThreadViewHandler_Click(object sender, EventArgs e) {
            ...
            if (sender == null) {
                ...
                sendText = ((ToolStripMenuItem)sender).Text;
            }
               ...
               //Bilge.FurtherInfo("Filename for default filter load is " + filterName);
                ...
                if (File.Exists(filterName)) {
                   ...
                   //Bilge.Warning("Default filter filename could not be found, ignoring filter load.", "Filename:" + filterName);
                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.DefaultFilterFileNotFound, ViewSupportManager.UserMessageType.WarningMessage, filterName);
                }
            }
           ...
           //Bilge.Dump(ViewerStatus);
            ...
            try {
                using (IsolatedStorageFile isoStore = IsolatedStorageFile.GetStore(IsolatedStorageScope.User | IsolatedStorageScope.Assembly, null, null)) {
                    using (IsolatedStorageFileStream storeStream = new IsolatedStorageFileStream(MEX_SETTINGSFILENAME, FileMode.Create, isoStore)) {
                        this.ViewerStatus.SaveViewerStatus(storeStream);
                        storeStream.Close();
                    } // End using the settings file in the iso store.

                    using (IsolatedStorageFileStream storeStream2 = new IsolatedStorageFileStream(MEX_OPTIONSFILENAME, FileMode.Create, isoStore)) {
                ...
                using (IsolatedStorageFile isoStore = IsolatedStorageFile.GetStore(IsolatedStorageScope.User | IsolatedStorageScope.Assembly, null, null)) {
                   ...
                   //Bilge.Log("Loading Viewer Status, importing Core.Options from IsolatedStorage\\" + MEX_OPTIONSFILENAME);
                    ...
                    if (isoStore.GetFileNames(MEX_OPTIONSFILENAME).Length == 1) {
                        using (IsolatedStorageFileStream storeStream = new IsolatedStorageFileStream(MEX_OPTIONSFILENAME, FileMode.Open, isoStore)) {
                            try {
                                if (storeStream.Length > 0) {
                                    MexOptions mo = MexOptions.LoadFromFile(storeStream);
                            ...
                            if (recover) {
                                ...
                                } catch (IsolatedStorageException isox) {
                    ...
                    if (isoStore.GetFileNames(MEX_SETTINGSFILENAME).Length == 1) {
                        using (IsolatedStorageFileStream storeStream = new IsolatedStorageFileStream(MEX_SETTINGSFILENAME, FileMode.Open, isoStore)) {
                            try {
                                if (storeStream.Length > 0) {
        ...
        internal void RefreshQuickFilterList() {
           ...
           //Bilge.Assert(Directory.Exists(MexCore.TheCore.Options.FilterAndHighlightStoreDirectory), "The filter and higlight store directory MUST exist");
            ...
            foreach (string s in matchedFilters) {
                cboMainViewQuickFilterChange.Items.Add(Path.GetFileNameWithoutExtension(s));
            }
            cboMainViewQuickFilterChange.Items.Add(" > Refresh List <");
        }

        internal void ApplyViewerStatus() {
            this.Top = ViewerStatus.YLoc;
            this.Left = ViewerStatus.XLoc;
            this.Width = ViewerStatus.Width;
            this.Height = ViewerStatus.Height;
        ...
        private void mnuFileOpen_Click(object sender, System.EventArgs e) {
            using (frmOpenDialog ofd = new frmOpenDialog()) {
                ofd.InitialiseDialog(ViewerStatus.LastLoadedFile, ViewerStatus.FileMostRecentlyUsedList);
                if (ofd.ShowDialog() == DialogResult.OK) {
                    FileImportMethod fim = ofd.GetFileImportMethod();
                    bool asynch = ofd.GetAsynchOK();

                    MexCore.TheCore.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.ImportFileStarts, ViewSupportManager.UserMessageType.InformationMessage, ofd.Filename);

                    Job_LoadTraceFile jltf = new Job_LoadTraceFile(ofd.Filename, fim);
                    jltf.AssignIdentToProcess = ofd.GetLabelIdent();
        ...
        private void ApplicationShutdown() {
           ...
           //Bilge.Log("Mex::UI >> Removing Notifications ....");
            ...
            Core.ViewManager.RegisterForProcessListChanges -= this.UpdateProcessListsAndMenuCallback;
            Core.ViewManager.RegisterForSelectedProcessChanges -= this.UpdateFollowingSelectedProcessChange;
            Core.ViewManager.RegisterForSelectedProcessRefreshRequired -= this.UpdateFollowingNewEventsArrivedForSelectedApp;
            Core.ViewManager.RegisterForCurrentViewChanges -= this.UpdateCurrentViewAfterRequest;
        ...
        private void RespondToSelectedChangeRequest(object sender, System.EventArgs e) {
           //Bilge.Log("Mex::UI >> USer clicked menu item to change process.");
            ...
            int menuItemCount = 0;
            LinkLabel senderAsLL = sender as LinkLabel;
            ToolStripMenuItem senderAsTsMi = sender as ToolStripMenuItem;
           ...
           //Bilge.Assert(sender != null, " This method can not be called with a null sender as the tag of the calling object is required");
            ...
            int theDesiredIndex = -1;

            if (senderAsTsMi != null) {
                senderAsTsMi.Checked = true;
                theDesiredIndex = (int)senderAsTsMi.Tag;
            } else if (senderAsLL != null) {
           ...
           //Bilge.Assert(theDesiredIndex != -1, "The index for selecting the application should be set to a valid application when this method is called");
            ...
            while (mnuViewMenu.DropDownItems[menuItemCount] != mnuViewProcessListDivider) {
                ToolStripMenuItem nextItem = (ToolStripMenuItem)mnuViewMenu.DropDownItems[menuItemCount];
                nextItem.Checked = (theDesiredIndex == (int)nextItem.Tag);
                menuItemCount++;
            }

            foreach (LinkLabel llnext in processLinkLabelCache) {
        ...
        private void ChangeDisplayedProcess(int theDesiredIndex) {
            ...
            ProcessSummary ps = Core.ViewManager.SelectedTracedAppSummary;
        ...
        private void RefreshCurrentView(bool refreshMode) {
            ...
            if (tbcMainView.SelectedTab == tabTimingsView) {
                Core.ViewManager.RefreshView_Timings(tvwTimingInstances, this.currentAdditionalData);
            }
        ...
        private void RefreshProcessListCache() {
            ...
            ProcessSummary[] pss = GetProcessSummaries();
           ...
           //Bilge.Log("Mex::UI >> UpdateProcessListsAndMenuCallback, callback rebuilding cache of LinkLbls");
            ...
            for (int i = 0; i < pss.Length; i++) {
               //Bilge.Assert(pss[i].DisplayName != null, "Mex::UI >> ERROR >> ps.DisplayName should be valid or empty by the time we try adn write it to the screen");
                ...
                LinkLabel ll = new System.Windows.Forms.LinkLabel();
                ll.AutoSize = true;
                ll.Name = "llquickProc" + i.ToString();
                ll.TabStop = false;
                ...
                ToolStripMenuItem newProcessMenuItem = new ToolStripMenuItem(menuItemText, null, new EventHandler(RespondToSelectedChangeRequest));
                newProcessMenuItem.Tag = pss[i].InternalIndex;
        ...
        private void RedrawProcessLists(/*ProcessSummary[] pss*/) {
            ...
            using (Panel tempWorkingPanel = new Panel()) {
                int totalAvailablePanelWidth = this.pnlQuickProcessLinkHolder.Width - (OVERHEAD_EACHEND_DUE_TO_SCROLLERS * 2);
                int nextLLXCoordinate = LL_INITIAL_LEFTPOSITION;

                for (int llCount = this.processLinkLabelFirstOffset; llCount < processLinkLabelCache.Count; llCount++) {
                    ...
                    this.pnlQuickProcessLinkHolder.Controls.Add(processLinkLabelCache[llCount]);
        ...
        private void RemoveLinkLabelsFromPanel() {
            ...
            for (int i = 0; i < pnlQuickProcessLinkHolder.Controls.Count;) {
                Control c = pnlQuickProcessLinkHolder.Controls[i];
...
#if DEBUG
            int debugLoopProtect = 1000;
#endif

            while (!(mnuViewMenu.DropDownItems[0] is ToolStripSeparator)) {
        ...
        private void UpdateProcessListsAndMenuCallback(ProcessSummary[] pss) {
           ...
           //Bilge.Warning("This is innefficient, the datamanager goes to the trouble of telling us just those that have changed, we should use that");
            ...
            if (this.InvokeRequired) {
                ...
                this.Invoke(new voidmeth(RedrawProcessLists));
            } else {
        ...
        private delegate void voidmeth();

        private delegate void boolMeth(bool dummy);

        private delegate void stringraymeth(string[] dummy);

        private delegate void pssraymeth(ProcessSummary[] dummy);

        private delegate void pssinglemeth(ProcessSummary dummy);
           ...
           //Bilge.Log("UI Recieved update notification of selected process changed ", ps.DisplayName);
            if (this.InvokeRequired) {
               //Bilge.FurtherInfo("Switching back to the UI thread to perform update following selected process change");
                this.Invoke(new pssinglemeth(RealUpdateFollowingSelectedProcessChange), ps);
            } else {
        ...
        private void UpdateCurrentViewAfterRequest(bool incrementalOK) {
           //Bilge.Log("UI recieved update notification requesting a refresh of the current process", "Incremental value " + incrementalOK.ToString());
            ...
            TimeSpan elapsed = DateTime.Now - m_lastRefreshNotfication;
               ...
               //Bilge.VerboseLog("Refresh being ignored because came too frequently, queueing up a refresh request for later");
                ...
                if (this.InvokeRequired) {
                   //Bilge.FurtherInfo("Switching back to UI thread to perform refresh");
                    this.Invoke(new boolMeth(RefreshCurrentView), incrementalOK);
                } else {
                    RefreshCurrentView();
                }
            }
        ...
        private void UpdateFollowingNewEventsArrivedForNonTracedApp(bool incrementalOk) {
            ...
            if (Core.Options.AutoRefresh) {
                if (this.InvokeRequired) {
                    this.Invoke(new voidmeth(RefreshViewIncrementallyOnChange));
                } else {
        ...
        private void PositionDialogOverView(Form dialog, bool full) {
            if (full) {
                dialog.Left = this.Left + this.tbcMainView.Left + 2;
                dialog.Top = this.Top + this.tbcMainView.Top + 2;
                dialog.Width = this.tbcMainView.Width;
                dialog.Height = this.tbcMainView.Height;
            } else {
                dialog.Left = this.Left + 2;
                dialog.Top = this.Top + 35;
            }
        }

        private void UpdateFollowingOptionsChangeNotification(string[] ListOfFilters) {
            if (this.InvokeRequired) {
                this.Invoke(new stringraymeth(RefreshScreenFromOptionsView), ListOfFilters);
            } else {
        ...
        private void Form1_Load(object sender, System.EventArgs e) {
           //Bilge.E("Mex::UI >> Form_Load");
            try {
                ...
                Core.ViewManager.RegisterForProcessListChanges += new ViewSupportManager.ProcessListChanged(this.UpdateProcessListsAndMenuCallback);
                Core.ViewManager.RegisterForSelectedProcessChanges += new ViewSupportManager.SelectedProcessChanged(this.UpdateFollowingSelectedProcessChange);
                Core.ViewManager.RegisterForSelectedProcessRefreshRequired += new ViewSupportManager.SelectedProcessRefreshRequired(this.UpdateFollowingNewEventsArrivedForSelectedApp);
                Core.ViewManager.RegisterForCurrentViewChanges += new ViewSupportManager.CurrentViewChanged(this.UpdateCurrentViewAfterRequest);
        ...
        private void lvwProcessView_SelectedIndexChanged(object sender, System.EventArgs e) {
            ...
            ListViewItem lvi = lvwProcessView.SelectedItems[0]; // we dont allow multi select therefore this should only be one
        ...
        private void SelectTreeView() {
            ...
            KeyDisplayRepresentation threadKey = ((KeyDisplayRepresentation)cboAppThreads.SelectedItem);
        ...
        private void ExecuteTreeViewRefreshCommand() {
            ...
            List<KeyDisplayRepresentation> threadIds = Core.ViewManager.GetThreadNameListForSelectedApp();
            KeyDisplayRepresentation selectedThread = (KeyDisplayRepresentation)cboAppThreads.SelectedItem;
        ...
        private void ExecuteCrossProcessViewRefreshCommand() {
            ...
            foreach (ProcessSummary ps in pss) {
                clbAllKnownProcesses.Items.Add(ps, checkedIdxs.Contains(ps.InternalIndex));
            }
        ...
        private void ExecuteSourceViewRefreshCommand() {
            ...
            if (cboSelectedThread.Items.IndexOf(lastthread) >= 0) {
                cboSelectedThread.SelectedIndex = cboSelectedThread.Items.IndexOf(lastthread);
            } else {
                cboSelectedThread.SelectedIndex = 0;
            }
        ...
        private void ExecuteThreadViewRefreshCommand() {
            ...
            if (threadIds.Count == 0) {  // There werent any threads to select at all.
               ...
               //Bilge.Log("WARNING >>Failed to execute a thread view referesh. There were no threads checked therefore the update could not proceeed");
                Core.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.NoThreadsSelectedForView, ViewSupportManager.UserMessageType.WarningMessage, null);
                return;
            }
            ...
            if (clbThreadList.CheckedItems.Count == 0) {
                ...
                selectedThreads.Add(threadIds[0]);
            } else {
            ...
            foreach (KeyDisplayRepresentation nextThread in threadIds) {
                ...
                clbThreadList.Items.Add(nextThread, nextThreadIsSelected);
            }
        ...
        private void btnSetFilter_Click(object sender, System.EventArgs e) {
           //Bilge.EnterSection("Setting Filter Options");
            using (frmProcViewFilter fpvf = new frmProcViewFilter()) {
        ...
        private void mnuFindFind_Click(object sender, System.EventArgs e) {
            using (frmFindDialog ffd = new frmFindDialog()) {
                this.PositionDialogOverView(ffd, false);
                ...
                if (ffd.ShowDialog() == DialogResult.OK) {
                    ...
                    if (ffd.GetLocationType() == frmFindDialog.FindMatchLocationType.CurrentPhysicalView) {
                        ...
                        } else if (tbcMainView.SelectedTab == tabODSView) {
        ...
        private void btnSemiPurgeCurrent_Click(object sender, System.EventArgs e) {
            ...
            if (tbcMainView.SelectedTab == tabCrossProcessView) {
                Core.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.GeneralUIError, ViewSupportManager.UserMessageType.WarningMessage, "Unable to clear a process from cross process view.");
                return;
            }
        ...
        private void lvwProcessView_DoubleClick(object sender, System.EventArgs e) {
            ListView chosenListView = (ListView)sender;
            ...
            ListViewItem lvi = chosenListView.SelectedItems[0]; // we dont allow multi select therefore this should only be one
           //Bilge.Assert(lvi != null, "This should not happen, selected index changed yet there was nothing selected but the time that we looked");
           //Bilge.Assert(lvi.Tag != null, "The tag should refer to the global index of the item, this entry was not inserted correctly into the grid");
            ...
            ViewSupportManager.ExtendedDetailsMode msgType = Core.ViewManager.DetermineMessageTypeForView(ref idx);
            switch (msgType) {
                case ViewSupportManager.ExtendedDetailsMode.Exception:
                    using (frmExceptionDetails fed = new frmExceptionDetails()) {
                        this.PositionDialogOverView(fed, true);
        ...
        private void DisplayErrorMessageExtendedDetails(long idx) {
            ...
            ErrorDialogData edd = Core.ViewManager.GetErrorFurtherInformation(idx);
            if (edd != null) {
                // If it is null then an internal error occured.
                //frmListenerErrorDialog.RenderErrorDialog(edd, false);
            } else {
                Core.ViewManager.AddUserNotificationMessageByIndex(ViewSupportManager.UserMessages.InvalidDataStructureError, ViewSupportManager.UserMessageType.WarningMessage, "Error detail could not be found in selected process.");
            }
        }
        ...
        private static void DisplayAssertionExtendedDetails(long idx) {
            AssertionPopulationData apd = Core.ViewManager.GetAssertionFurtherInformation(idx);
            //frmInteractiveAssertionDialog.MexRenderAssertionDialog(apd);
        }
        ...
        private void ShowOptionsScreen() {
            using (frmMexOptionsScreen fmos = new frmMexOptionsScreen()) {
                this.PositionDialogOverView(fmos, false);
           ...
           //Bilge.E("Mex::UI >> Form_Closed");
            ...
            try {
                ...
                if (ViewerStatus != null) {
                    ViewerStatus.Height = this.Height;
                    ViewerStatus.Width = this.Width;
                    ViewerStatus.XLoc = this.Left;
                    ViewerStatus.YLoc = this.Top;
        ...
        private void duplicateThisProcessToolStripMenuItem_Click(object sender, EventArgs e) {
           //Bilge.Assert(lvwProcessView.SelectedItems.Count > 1, " Invalid call to set index range");
            frmCreateDuplicate fcd = new frmCreateDuplicate();
        ...
        private void duplicateToolStripMenuItem_Click(object sender, EventArgs e) {
           //Bilge.Assert(lvwProcessView.SelectedItems.Count > 1, " Invalid call to set index range");
            ...
            long currentEndIndex = (long)lvwProcessView.SelectedItems[lvwProcessView.SelectedItems.Count - 1].Tag;
        ...
        private void ApplySelectionToFilter(long nextIdx, bool include, bool doThread, bool doLocation, bool doModule, bool doClass) {
            EventEntry ee = MexCore.TheCore.ViewManager.GetEventEntryFromSelectedAppByIndex(nextIdx);
            KeyDisplayRepresentation kdr = MexCore.TheCore.ViewManager.SelectedTracedApp.GetThreadDisplayValue(ee);

            List<KeyDisplayRepresentation> allThreadValues = new List<KeyDisplayRepresentation>();

            MexCore.TheCore.ViewManager.CurrentFilter.BeginFilterUpdate();
            if (doThread) {
                if (include) {
                    allThreadValues = Core.ViewManager.GetThreadNameListForSelectedApp();
            ...
            if ((doClass) && (ee.MoreLocationData.Length > 0)) {
                ...
                if (idx > 0) {
                    string thisEntrysClass = ee.MoreLocationData.Substring(0, idx);
        ...
        private void mnuViewPromoteCurrent_Click(object sender, EventArgs e) {
            ...
            frmViewInAWindow fvw = new frmViewInAWindow();
            this.PositionDialogOverView(fvw, true);
            if (m_PromotedViews == null) {
        ...
        private void copyToolStripMenuItem_Click(object sender, EventArgs e) {
            StringBuilder clippy = new StringBuilder();

            foreach (ListViewItem lvi in lvwProcessView.SelectedItems) {
                clippy.Append(string.Format("{0},{1},{2},{3},{4}" + Environment.NewLine, lvi.Text, lvi.SubItems[0].Text, lvi.SubItems[1].Text, lvi.SubItems[2].Text, lvi.SubItems[3].Text));
            }
        ...
        private void tmrUpdateUI_Tick(object sender, EventArgs e) {
            ...
            sbrMainBar.Panels["sbrWaitingMsgs"].Text = "Q[ " + s + " ]";

            s = Core.ViewManager.LastUserNotificationMessage;
            sbrMainBar.Panels["sbpMessageArea"].Text = s;

            s = Core.ViewManager.LastIssuedGlobalIndex;
            sbrMainBar.Panels["sbpLastGindex"].Text = s;

            s = Core.ViewManager.LastMessageRecievedTime;
            sbrMainBar.Panels["sbpRHS"].Text = "at " + s;
        ...
        private void RefreshQuickIcons() {
            if (Core.Options.AutoScroll) {
                btnAutoScrollQuick.ImageIndex = 39;
            } else {
                btnAutoScrollQuick.ImageIndex = 41;
            }

            if (Core.Options.AutoRefresh) {
                btnAutoRefreshQuick.ImageIndex = 40;
            } else {
                btnAutoRefreshQuick.ImageIndex = 42;
            }
        }

        private void SetRefreshTime(int seconds) {
            if ((seconds == 1) || (seconds == 2) || (seconds == 5)) {
        ...
        private void btnAdditional_Click(object sender, EventArgs e) {
            if (tbcMainView.SelectedTab == tabTimingsView) {
                using (frmTimingsAdditionalScreen tads = new frmTimingsAdditionalScreen()) {
                    tads.SetScreenFromAddtionalData(currentAdditionalData);
                    if (tads.ShowDialog() == DialogResult.OK) {
                        this.currentAdditionalData = tads.GetAdditionalDataFromScreen();
                    }
                }
                return;
            }
            ...
            using (frmMexUserMessages fmum = new frmMexUserMessages()) {
        ...
        private void lvwThreadView_SelectedIndexChanged(object sender, EventArgs e) {
            ...
            // They are interacting with the process view and selecting different entries
            // we need to identify the one that they selected and present sensible information in the bottom half of the screen.
            ListViewItem lvi = lvwThreadView.SelectedItems[0]; // we dont allow multi select therefore this should only be one
           //Bilge.Assert(lvi != null, "This should not happen, selected index changed yet there was nothing selected but the time that we looked");
           //Bilge.Assert(lvi.Tag != null, "The tag should refer to the global index of the list view item, if its null its been incorrectly inserted into the list view.");
        ...
        private void button1_Click(object sender, EventArgs e) {
            using (frmDiagnosticsScreen fds = new frmDiagnosticsScreen()) {
                fds.ShowDialog();
            }
        }
